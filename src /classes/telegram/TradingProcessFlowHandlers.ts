...
export interface ITradingProcessFlow {
     ...
     * @param {Session} session - The current session
     */
    handleContractTypeSelection(chatId: number, text: string, session: Session): void;
...
export class TradingProcessFlowHandlers implements ITradingProcessFlow {
    ...
    public async handleMarketSelection(chatId: number, text: string, session: Session): Promise<void> {
        ...
        session.bot.tradingOptions.step = CONSTANTS.SESSION_STEPS.SELECT_CONTRACT_TYPE;
        await this.sessionService.updateSessionWithChatId(chatId, session);
        this.keyboardService.showContractTypeKeyboard(chatId, session.bot.tradingOptions.market);
    }
     ...
     * @public
     */
    public async handleContractTypeSelection(chatId: number, text: string, session: Session): Promise<void> {
        session.bot.tradingOptions.contractType = text;
