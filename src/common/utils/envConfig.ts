...
export const env = cleanEnv(process.env, {
  ...
  CONNECTION_ATTEMPTS_TIMEOUT: num({ devDefault: testOnly(200) }),
  CONNECTION_ATTEMPTS_TIMEOUT_INCREMENT: num({ devDefault: testOnly(200) }),
  ...
  DERIV_APP_OAUTH_CHANNEL: str({ devDefault: testOnly("") }),
  DERIV_APP_LOGIN_URL: str({ devDefault: testOnly("") }),
  DERIV_APP_OAUTH_URL: str({ devDefault: testOnly("") }),
  ...
  MONGODB_BACKUP_PATH: str({ devDefault: testOnly("") }),
  DB_SERVER_SESSIONS_DATABASE: str({ devDefault: testOnly("") }),
  DB_SERVER_SESSIONS_DATABASE_COLLECTION: str({ devDefault: testOnly("") }),
  DB_SERVER_SESSIONS_DATABASE_TTL: num({ devDefault: testOnly(3600) }),
  DB_TG_BOT_SESSIONS_DATABASE: str({ devDefault: testOnly("") }),
  DB_TG_BOT_SESSIONS_DATABASE_COLLECTION: str({ devDefault: testOnly("") }),
});
