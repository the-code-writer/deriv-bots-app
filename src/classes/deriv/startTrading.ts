 ...
 * @param {ContractType} session.contractType - The type of contract to purchase
...
while (this.isTrading) {
    try {
        ...
        const tradeResult = await this.executeTrade(session.contractType);
...
if (!session.contractType || !Object.values(ContractType).includes(session.contractType)) {
    errors.push(`Invalid contractType: ${session.contractType}`);
}
...
private async initializeTradingSession(session: any): Promise < void> {
    ...
    this.currentContractType = session.contractType;
 ...
 * @private
 */
private async executeTrade(contractType: ContractType): Promise < ITradeData > {
    this.consecutiveTrades++;
    this.totalNumberOfRuns++;

    logger.info(`Executing trade #${this.totalNumberOfRuns} (${contractType})`);

    const tradeData = await this.purchaseNextContract(contractType);
...
private prepareNextTrade(tradeData: ITradeData): void {
    ...
    this.currentContractType = this.originalContractType;
    this.currentRecoveryTradeIndex = 0;
} else {
    ...
    if (this.currentRecoveryTradeIndex > 2) {
        this.currentContractType = this.getAlternativeStrategy();
    }
}
}
